...
kcs_load_plugin() {
  ...
  if ! [ -d "$target" ]; then
    ...
    mkdir -p "$target" || return "$KCS_ERR_TIDY_FAILED"
  fi
  ...
  if ! [ -f "$filepath" ] && ! "$_KCS_LOCAL"; then
    ...
    for registry in "${KCS_REGISTRIES[@]}"; do
      ...
      if ! curl -sSL -w "%{http_code}" -o "$filepath" "$download_url" >"$http_status_file"; then
        ...
        rm "$http_status_file" || return "$KCS_ERR_TIDY_FAILED"
        continue
      fi
      ...
      if [ "$http_status" -ge 400 ]; then
        ...
        rm "$filepath" || return "$KCS_ERR_TIDY_FAILED"
        continue
      fi
    done
  fi
